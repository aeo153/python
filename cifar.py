...
cifar_dir = "D:/cvdataset/cifar-10-batches-py/"
...
def cifar10_img(file_dir):
    for i in range(1, 6):
        ...
        for j in range(10000):
            ...
            img_name = train_dir + str(data_dict[b'labels'][j]) + str((i) * 10000 + j) + '.png'
    ...
    for m in range(10000):
        ...
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # 要改成不同的形式的文件只需要将文件后缀修改即可
        img_name = test_dir + str(test_dict[b'labels'][m]) + str(10000 + m) + '.png'
        cv2.imwrite(img_name, img)
    print(test_data_name + ' is done')
    print('Finish transforming to image')

if __name__ == '__main__':
    cifar10_img(cifar_dir)